...
class StandardROIHeadsPseudoLab(StandardROIHeads):
    ...
    def forward(
        ...
        if (self.training and compute_loss) or compute_val_loss:
            if branch == "adain_supervised":
                losses, _, box_features = self._forward_box(features, proposals, compute_loss, compute_val_loss, branch)
                return proposals, box_features, losses
            else:
    ...
    def _forward_box(
        ...
        # predictions[0].shape = [2048, 2] 表示每个proposal的种类
        # predictions[1].shape = [2048, 4] 表示每个proposal的box偏移
        predictions = self.box_predictor(box_features)

        if not branch == "adain_supervised":
            del box_features
