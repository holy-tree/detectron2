...
def build_detection_semisup_train_loader(cfg, mapper=None):
    ...
    # dataset_dicts是一个list 其中包含 file_name(路径) image_id  height  width  annotations{category_id bbox[] bbox_mode}  
    ...
    # dataset 是一个 torch.utils.data.dataset
    dataset = DatasetFromList(label_dicts, copy=False)

    if mapper is None:
        mapper = DatasetMapper(cfg, True)
    # dataset 同样是 torch.utils.data.dataset
...
def build_detection_semisup_train_loader_two_crops(cfg, mapper=None):
    ...
    if cfg.DATASETS.CROSS_DATASET:  # cross-dataset (e.g., coco-additional)
        label_dicts = get_detection_dataset_dicts(
            names=cfg.DATASETS.TRAIN_LABEL
        )
        unlabel_dicts = get_detection_dataset_dicts(
            names=cfg.DATASETS.TRAIN_UNLABEL,
        )
    #上述 label_dicts 和 unlabel_dicts 都是 list[dist] 格式

    # else:  # different degree of supervision (e.g., COCO-supervision) 只有一个数据集 分开监督集和无监督集(无用)
    #     dataset_dicts = get_detection_dataset_dicts(
    #         cfg.DATASETS.TRAIN,
    #         filter_empty=cfg.DATALOADER.FILTER_EMPTY_ANNOTATIONS,
    #         min_keypoints=cfg.MODEL.ROI_KEYPOINT_HEAD.MIN_KEYPOINTS_PER_IMAGE
    ...
    #将 list[] 类型转化为 torch.utils.data.dataset 格式
    ...
    # if mapper_label is None:
    #     mapper_label = DatasetMapper(cfg, True)
    # if mapper_unlable is None:
    #     mapper_unlable = DatasetMapper(cfg, True)
    if mapper is None:
        mapper = DatasetMapper(cfg, True)
    ...
    # print(label_dataset)
    # print(len(label_dataset.__getitem__(0)))
    # print(label_dataset.__getitem__(0)[0]['image'].shape)
    # label_image = label_dataset.__getitem__(0)[0]['image']
    # unlabel_image = unlabel_dataset.__getitem__(0)[0]['image']

    # 查看强弱增强图像
    
    # label_image = label_image.clone().detach().double().to(torch.device('cpu'))
    # unlabel_image = unlabel_image.clone().detach().double().to(torch.device('cpu'))
    # label_image = np.ascontiguousarray(label_image.numpy().transpose((1,2,0)))
    # unlabel_image = np.ascontiguousarray(unlabel_image.numpy().transpose((1,2,0)))
    # cv2.imwrite(f"./label_strong_image.jpg", label_image)
    # cv2.imwrite(f"./unlabel_strong_image.jpg", unlabel_image)
    # quit()
...
def build_semisup_batch_data_loader_two_crop(
    ...
    *,
    ...
    if aspect_ratio_grouping:
        ...
        # 测试dataloader
        # print(label_data_loader)
        # for i in label_data_loader:
        #     print(i)
        #     from  torchvision import utils as vutils
        #     strong_tensor = i[0]['image'].clone().detach().double().to(torch.device('cpu'))
        #     weak_tensor = i[1]['image'].clone().detach().double().to(torch.device('cpu'))
        #     vutils.save_image(strong_tensor, './strong.jpg', normalize=True)
        #     vutils.save_image(weak_tensor, './weak.jpg', normalize=True)
        #     break
        # quit()
